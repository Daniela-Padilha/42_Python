# 🐍 42 Python Modules — Growing Code

This repository contains my solutions to the **42 Python Piscine — Growing Code** modules (00 to 09).  
The curriculum introduces Python through a progressive, data-driven approach using garden-themed datasets, focusing on clean code, reasoning, and real-world data manipulation.

Each module builds upon the previous one, moving from Python fundamentals to object-oriented programming, data analysis, and advanced language features.

---
> [NOTE]
> FOR 42 STUDENTS
> These modules were not evaluated, and as such might not comply with evaluation rules.

## 🌿 Module 00 — Python Fundamentals Through Garden Data

Introduction to Python syntax and basic programming concepts using simple garden-related data.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Python execution, basic output, environment setup                        |
| Variables, basic data types, arithmetic operations                       |
| String manipulation and formatting                                       |
| Conditional logic applied to garden data                                 |
| Functions and simple data processing                                     |

---

## 🌱 Module 01 — Object-Oriented Garden Systems

Build a comprehensive digital garden ecosystem while discovering advanced
Python concepts.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Classes, __init__, OOP, encapsulation                                    |
| f-string                                                                 |
| Setters and getters                                                      |
| Tuples, arg unpacking and list comprehension                             |
| Inheritance, super()                                                     |
| Nested classes, instance method, class method, static method             |

---

## 🌼 Module 02 — Data Engineering for Smart Agriculture

Build resilient data pipelines for your smart garden!

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Exception handling                                                       |
| Custom Exceptions                                                        |
| Dictionaries as structured datasets                                      |
| Polymorphism                                                             |
| Iterating and querying collections                                       |

---

## 🌾 Module 03 — Functional Programming & Transformations

Applying functional programming concepts to transform and analyze datasets.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Lambda functions                                                         |
| `map`, `filter`, and `reduce`                                            |
| List and dictionary comprehensions                                       |
| Generator expressions                                                    |
| Data pipelines and functional-style logic                                |

---

## 🌳 Module 04 — Files, Parsing & Exceptions

Reading, writing, and safely handling garden data stored in files.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| File reading and writing                                                 |
| Parsing structured text files                                            |
| Exception handling (`try`, `except`, `finally`)                          |
| Custom exceptions                                                        |
| Robust, fault-tolerant programs                                          |

---

## 🪴 Module 05 — Object-Oriented Programming

Modeling garden entities using classes and objects.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Defining classes and creating objects                                    |
| Attributes and instance methods                                          |
| Constructors (`__init__`)                                                |
| Encapsulation and data protection                                        |
| Object representation (`__str__`, `__repr__`)                            |

---

## 🌺 Module 06 — Inheritance & Polymorphism

Extending garden models through inheritance and polymorphic behavior.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Base and derived classes                                                 |
| Method overriding                                                        |
| `super()` and constructor chaining                                       |
| Polymorphism in practice                                                 |
| Abstract-like class design                                               |

---

## 🌻 Module 07 — Advanced OOP & Operator Behavior

Enhancing object models with advanced Python OOP features.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Operator overloading                                                     |
| Magic methods (`__eq__`, `__lt__`, etc.)                                 |
| Composition vs inheritance                                               |
| Class attributes vs instance attributes                                  |

---

## 🍃 Module 08 — Decorators & Context Management

Adding behavior and managing resources cleanly and safely.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Function decorators                                                      |
| Decorators with parameters                                               |
| Logging and performance measurement                                      |
| Context managers (`with` statement)                                      |
| Resource and state management                                            |

---

## 🌍 Module 09 — Iteration, Generators & Advanced Concepts

Final module focused on efficient data processing and Python internals.

| Concepts                                                                 |
|--------------------------------------------------------------------------|
| Iterators and iterable protocols                                         |
| Generator functions                                                      |
| Closures and lexical scoping                                             |
| Dynamic behavior and metaprogramming basics                              |
| Performance considerations and clean architecture                        |

---

## 🧪 Tests
Where applicable, example inputs or test cases are included to validate correctness and expected behavior.

---

## ▶️ Run

Each exercise can be executed with:
```bash
python3 exXX.py
